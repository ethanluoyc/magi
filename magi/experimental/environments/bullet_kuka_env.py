...
class KukaDiverseObjectEnv(KukaGymEnv):
  ...
  def __init__(self,
    ...
    if self._renders:
      ...
      optionstring = '--width={} --height={}'.format(self._width, self._height)
...
class TimeStepObservationWrapper(gym.Wrapper):
  """Add current time step observation to the obs."""

  def __init__(self, env):
    super().__init__(env)
    self.observation_space = spaces.Dict({
        'observation': self.unwrapped.observation_space,
        't': spaces.Box(np.array(0), np.array(100), dtype=np.int32),
    })
  ...
  def reset(self):
    ...
    return {
        'observation': obs,
        't': np.array(self.unwrapped._env_step),
    }

  def step(self, action):
    obs, reward, done, info = super().step(action)
    obs = {'observation': obs, 't': np.array(self.unwrapped._env_step)}
