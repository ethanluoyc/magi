...
This describes the set of interfaces used in Magi.
They are adapted from Acme. See acme/core.py.
For now, they are kept consistent with Acme
but may be subject to change in the future as the library develops.

We include below the license from the original acme repository.
"""
...
class Actor(abc.ABC):
  ...
  The actors are assumed to interact with environments that adheres to the
  dm_env.Environment interface.
  For gym environments, users and use the acme.wrappers.GymWrapper
  to wrap the environment so that they are consistent with this interface.
  We may consider creating our own interfaces for the timesteps and actions in furure.
  """
