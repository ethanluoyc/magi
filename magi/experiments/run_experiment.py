...
from magi.experiments import config
...
def run_experiment(
    ...
    train_counter = counting.Counter(parent_counter, prefix="train", time_delta=0.0)
    train_logger = experiment.logger_factory("train", train_counter.get_steps_key(), 0)
    ...
    actor = _LearningActor(
        actor, learner, dataset, replay_tables, rate_limiters_max_diff
    )
    ...
    eval_logger = experiment.logger_factory("eval", eval_counter.get_steps_key(), 0)
