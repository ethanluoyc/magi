...
# Loss functions
def make_critic_loss_fn(encoder_apply, actor_apply, linear_apply, critic_apply, discount):
    @jax.jit
    def _loss_critic(
        ...
        next_last_conv = jax.lax.stop_gradient(encoder_apply(params_critic["encoder"], next_state))
        next_action, next_log_pi = _sample_action(actor_apply, params_actor, next_last_conv, key)
        target = _calculate_target(linear_apply, critic_apply, params_critic_target, log_alpha, reward, done, next_last_conv, next_action, next_log_pi, discount)
        q_list = _calculate_value_list(linear_apply, critic_apply, params_critic, last_conv, action)
...
def _calculate_target(
    ...
    next_q = _calculate_value(linear_apply, critic_apply, params_critic_target, next_state, next_action)
...
def _calculate_value(
    ...
    return jnp.asarray(_calculate_value_list(linear_apply, critic_apply, params_critic, state, action)).min(axis=0)
...
def make_actor_loss_fn(encoder_apply, actor_apply, linear_apply, critic_apply):
    ...
    def _loss_actor(
        ...
        mean_q = _calculate_value(linear_apply, critic_apply, params_critic, last_conv, action).mean()
...
def make_ae_loss_fn(encoder_apply, linear_apply, decoder_apply, lambda_latent, lambda_weight):
    def _loss_ae(
        params_ae: hk.Params,
        state: np.ndarray,
        key: jnp.ndarray,
...
class SAC_AE:
    ...
    def __init__(
        ...
        # Setup losses
        self._loss_critic = make_critic_loss_fn(self.encoder.apply, self.actor.apply, self.linear.apply, self.critic.apply, self.discount) 
        self._loss_actor = make_actor_loss_fn(self.encoder.apply, self.actor.apply, self.linear.apply, self.critic.apply)
        self._loss_ae = make_ae_loss_fn(self.encoder.apply, self.linear.apply, self.decoder.apply, self.lambda_latent, self.lambda_weight)
    ...
    def step(self, env, state):
        ...
        if self.agent_step <= self.start_steps:
            ...
            action = self.select_action(state, is_eval=False)
