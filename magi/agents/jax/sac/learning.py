...
def compute_target(
    ...
    action_tp1_base_dist = tfd.MultivariateNormalDiag(
        loc=mean_tp1,
        scale_diag=jnp.exp(logstd_tp1)
    )
    action_tp1_dist = tfd.TransformedDistribution(
        action_tp1_base_dist, tfb.Tanh()
    )
    ...
    return (
        r_t +
        discount * d_t * (
            jnp.minimum(q1,q2) - alpha * next_log_pis
        )
    )

def q_value_loss(policy,
    critic, 
    rng,
    policy_params,
    critic1_params,
...
def policy_loss(
    policy, critic, policy_params, critic1_params, critic2_params,
    rng,
    o_t,
    a_t,
    alpha,
    ...
    return jax.lax.stop_gradient(alpha) * mean_log_pi - mean_q, jax.lax.stop_gradient(mean_log_pi)

def polyak_update(old_params, new_params, rate):
    return jax.tree_multimap(
        lambda o, n: rate * o + (1 - rate) * n, old_params, new_params
    )
...
class SACLearner(acme.Learner):
    def __init__(
        ...
        dummy_obs = tree.map_structure(lambda x: jnp.expand_dims(x, 0), dummy_from_spec(observation_spec))
        dummy_action = tree.map_structure(lambda x: jnp.expand_dims(x, 0), dummy_from_spec(action_spec))
        ...
        self._logger = loggers.TerminalLogger('learner', time_delta=10.)
        ...
        def sgd_step(batch, rng, params, opt_state):
            ...
            (critic_loss_, (grad_critic1, grad_critic2)) = (jax.value_and_grad(q_value_loss, argnums=(5, 6))(
            ...
            critic2_updates, critic2_opt_state = self._opt_critic.update(
                ...
                critic2_opt_state,
                critic2_params)
            critic1_params = optax.apply_updates(critic1_params, critic1_updates)  # update the parameters.
            critic2_params = optax.apply_updates(critic1_params, critic2_updates)  # update the parameters.

            # Update policy by one step of gradient descent
            (actor_loss, log_pi_mean), grad_policy = (jax.value_and_grad(policy_loss, argnums=2, has_aux=True)(
                self._policy_network,
                self._critic_network,
                policy_params,
                critic1_params,
            ...
            policy_updates, policy_opt_state = self._opt_actor.update(
                ...
                policy_opt_state,
                policy_params)
            policy_params = optax.apply_updates(policy_params, policy_updates)  # update the parameters.
            # Update target networks
            critic1_target_params = polyak_update(critic1_target_params, critic1_params, self._rate)
            critic2_target_params = polyak_update(critic2_target_params, critic2_params, self._rate)

            # Update alpha(
            log_alpha_loss, grad_log_alpha = jax.value_and_grad(self._loss_alpha)(log_alpha, log_pi_mean)
    ...
    def step(self):
        ...
        losses, self._params, self._opt_state = self._sgd_step(batch, next(self._rng), self._params, self._opt_state)
        self._logger.write(losses)