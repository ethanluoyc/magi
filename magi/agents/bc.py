...
class BCLearner(acme.Learner):
    ...
    def __init__(
        ...
        def sgd_step(
            ...
            loss_value, gradients = loss_and_grad(
                network.apply, state.policy_params, key_input, transitions
            )
        ...
        self._sgd_step = jax.jit(
            utils.process_multiple_batches(sgd_step, num_sgd_steps_per_step)
        )
