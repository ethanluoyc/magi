...
from magi.agents.archived.sac_ae.wrappers import ConcatFrameWrapper
...
def main(_):
  ...
  experiment_name = (f"sac_ae-{FLAGS.domain_name}-{FLAGS.task_name}_{FLAGS.seed}"
                     f"_{int(time.time())}")
  if FLAGS.wandb:
    import wandb
  ...
  decoder = hk.without_apply_rng(
      hk.transform(lambda f: networks.SACDecoder(spec.observations['pixels'].shape[-1])
                   (f)))
  linear = hk.without_apply_rng(hk.transform(lambda f: networks.SACLinear(50)(f)))
