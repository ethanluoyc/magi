...
def make_autoencoding_loss(encoder_forward, linear_forward, decoder_forward, *,
  ...
  def _loss_fn(params, obs, key):
    ...
    target = preprocess_state(obs, key)
...
class SACAELearner(core.Learner):
  ...
  def step(self):
    ...
    if self._step % self._actor_update_freq == 0:
      self.policy_params, self.policy_opt_state, loss_actor, mean_log_pi = (
          self._update_actor(self.policy_params, self.policy_opt_state, next(self._rng),
