...
class PlannerTest(parameterized.TestCase):
  ...
  def test_compute_n_step_return(self):
    ...
    return_prediction = planning._compute_n_step_return(
        next_fn,
        critic_fn,
        policy_fn,
        params,
    ...
    return_expected = (
        jnp.sum(jnp.square(actions[0])) + jnp.sum(jnp.square(actions[1])) *
        (discount**1) + jnp.sum(jnp.square(actions[2])) * (discount**2) +
