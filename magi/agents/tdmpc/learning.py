...
class TDMPCLearner(acme.Learner):
  ...
  def _compute_loss(
    ...
    # z_tp1, z_tp2 ... z_tpH
    (online_z_posterior,
     reward_pred), _ = hk.static_unroll(next_core, actions, z_prior[0])
    # [H, B, Z]
    ...
    consistency_loss = jnp.mean(
        _l2_loss(z_predictions[1:], z_target[1:]), axis=-1)
    ...
    chex.assert_equal_shape(
        [reward_loss, value_loss, priorities, consistency_loss])
    ...
    weighted_model_loss = optax.scale_gradient(weighted_model_loss,
                                               1.0 / horizon)
    ...
    # Compute policy loss
    policy_q1, policy_q2 = jax.vmap(self._networks.q,
                                    (None, 0, 0))(frozen_params, z_policy,
                                                  policy_actions)
  ...
  def _update(self, state, batch):
    ...
    (_, (priorities, metrics)), gradients = grad_fn(state.params,
                                                    state.target_params, batch,
                                                    key)

    metrics["grad_norm"] = optax.global_norm(gradients)

    updates, opt_state = self._optimizer.update(gradients, state.opt_state,
                                                state.params)
