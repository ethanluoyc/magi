...
class TDMPCActor(core.Actor):
  ...
  def select_action(
      self, observation: networks_lib.Observation) -> networks_lib.Action:
    if self._actor_steps < self._seed_steps and not self._evaluation:
      action, self._state = self._random_policy(self._params, observation,
                                                self._state)
    else:
      action, self._state = self._plan_step(self._params, observation,
                                            self._state)
