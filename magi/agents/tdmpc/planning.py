...
def td_mpc_planner(
  ...
  """Run the TD-MPC planner.
    ...
    Returns:
        mean: [H, A] array. The final mean parameter for adapted trajectory distribution.
        std: [H, A] array. the final std parameter for adapted trajectory distribution.
        scores: [k] array. The final normalized empirical value estimate.
  ...
  if n_policy_trajectories > 0:
    ...
    policy_actions = _rollout_policy(params, model_fn, policy_fn, z,
                                     rollout_key, horizon,
                                     n_policy_trajectories)

  # Repeat for sample trajectories
  z = jnp.tile(
      z, (n_sample_trajectories + n_policy_trajectories, 1))  # [N + N_pi, Z]
  ...
  def loop_body_fn(_, state):
    ...
    action_noise = jax.random.normal(
        action_key, (n_sample_trajectories, horizon, action_dims))  # [N, H, A]
    ...
    if policy_actions is not None:
      actions = jnp.concatenate([actions, policy_actions],
                                axis=0)  # [N + N_pi, H, A]
    ...
    chex.assert_shape([values],
                      (n_sample_trajectories + n_policy_trajectories,))
    ...
    new_mean = jnp.sum(
        weights * top_k_actions, axis=0) / (jnp.sum(scores, axis=0) + 1e-9
                                           )  # [H, A]
    new_std = jnp.sqrt(
        jnp.sum(weights * jnp.square(top_k_actions - new_mean), axis=0) /
        (jnp.sum(scores, axis=0) + 1e-9))  # [H, A]
  ...
  actions = top_k_actions[jax.random.choice(key, k, replace=True,
                                            p=scores)]  # [H, A]
