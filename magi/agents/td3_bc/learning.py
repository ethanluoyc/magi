...
class TD3BCLearner(acme.Learner):
  ...
  def __init__(
    ...
    def critic_loss_fn(
      ...
      noise = jnp.clip(
          jax.random.normal(random_key, transitions.action.shape) *
          policy_noise,
          -noise_clip,
          noise_clip,
      )

      next_action = policy_network.apply(policy_target_params,
                                         transitions.next_observation)
      next_action = next_action + noise
      next_action = jnp.clip(next_action, -1.0, 1.0)

      # Compute the target Q value
      q1_target, q2_target = critic_network.apply(critic_target_params,
                                                  transitions.next_observation,
                                                  next_action)
      q_target = jnp.minimum(q1_target, q2_target)
      q_target = jax.lax.stop_gradient(transitions.reward +
                                       transitions.discount * discount *
                                       q_target)
    ...
    def sgd_step(state: TrainingState, batch: reverb.ReplaySample):
      ...
      (policy_loss, policy_metrics), policy_grads = jax.value_and_grad(
          policy_loss_fn, has_aux=True)(state.policy_params,
      ...
      def update_policy_state():
        ...
        policy_target_params = soft_update(policy_params,
                                           state.policy_target_params)
