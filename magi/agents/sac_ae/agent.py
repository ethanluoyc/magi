...
def make_critic_loss_fn(encoder_apply, actor_apply, linear_apply, critic_apply,
  ...
  def _loss_critic(
    ...
    next_action, next_log_pi = actor_apply(
        params_actor, next_last_conv).sample_and_log_prob(key)
    # Compute target
    ...
    next_q1, next_q2 = critic_apply(params_critic_target['critic'],
                                    next_feature_target, next_action)
...
def make_actor_loss_fn(encoder_apply, actor_apply, linear_apply, critic_apply):
  ...
  def _loss_actor(
    ...
    action, log_pi = actor_apply(params_actor,
                                 last_conv).sample_and_log_prob(key)
...
def make_ae_loss_fn(
  ...
  def _loss_ae(params_ae: hk.Params, key: jnp.ndarray, observation: np.ndarray):
    ...
    return (
        loss_reconst + lambda_latent * loss_latent +
...
class SACAEAgent(core.Actor, core.VariableSource):
  ...
  def __init__(
    ...
    # Entropy coefficient.
    self._log_alpha = jnp.array(
        np.log(config.init_temperature), dtype=jnp.float32)
