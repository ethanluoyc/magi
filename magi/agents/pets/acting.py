...
class OptimizerBasedActor(core.Actor):
  ...
  def __init__(
    ...
    self._initial_solution = np.ones(shape=(planning_horizon, *action_spec.shape)) * (
  ...
  def select_action(self, observation: np.ndarray):
    ...
    key = next(self._rng)
    actions, extras = self._controller_fn(self._cost_fn, params, normalizer,
                                          observation, self._last_actions, key,
                                          self._goal)
    self._extras = extras
  ...
  def observe(self, action: np.ndarray, next_timestep: dm_env.TimeStep):
    ...
    self._replay.add(self._last_timestep.observation, action, next_timestep.observation,
...
class CEMOptimizerActor(OptimizerBasedActor):
  ...
  def __init__(
    ...
    super().__init__(spec, model_env, replay, variable_client, controller,
                     planning_horizon, num_particles, seed, num_initial_episodes)
