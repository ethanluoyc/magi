...
"""NumPy-based Replay buffer implementation."""
import dataclasses
import pathlib
...
def _consolidate_batches(batches: Sequence[TransitionBatch]) -> TransitionBatch:
  ...
  for i, b in enumerate(batches):
    ...
    dones[i] = b.dones
  return TransitionBatch(
      obs=obs, act=act, next_obs=next_obs, rewards=rewards, dones=dones)
...
class ReplayBuffer:
  ...
  @staticmethod
  def _check_overlap(segment1: Tuple[int, int], segment2: Tuple[int,
                                                                int]) -> bool:
    s1, e1 = segment1
    s2, e2 = segment2
  ...
  def close_trajectory(self):
    ...
    if self.cur_idx - self._start_last_trajectory > (len(self.obs) -
                                                     self.capacity):
      warnings.warn('A trajectory was saved with length longer than expected. '
  ...
  def sample_trajectory(self) -> Optional[TransitionBatch]:
    ...
    indices = np.arange(self.trajectory_indices[idx][0],
  ...
  def _batch_from_indices(self, indices: Sized) -> TransitionBatch:
    ...
    return TransitionBatch(
        obs=obs, act=action, next_obs=next_obs, rewards=reward, dones=done)
