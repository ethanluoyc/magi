...
def minimize_cem(
  ...
  def loop(state):
    ...
    population = jax.random.truncated_normal(
        key, lower=-2.0, upper=2.0, shape=population_shape)
    ...
    new_best_cost = jnp.where(best_costs[0] < best_cost, best_costs[0],
                              best_cost)
    new_best_solution = jnp.where(best_costs[0] < best_cost, elites[0],
                                  best_solution)
