...
class ModelBasedLearner(core.Learner):
  ...
  def __init__(
    ...
    def update(state: TraningState, x, a, xnext):
      ...
      value, grads = jax.value_and_grad(model.loss)(state.params, state.state,
                                                    x, a, xnext)
  ...
  def _evaluate(self, params: hk.Params, state: models.ModelState, iterator):
    ...
    for batch in iterator:
      batch_loss = self._model.evaluate(params, state, batch.obs, batch.act,
                                        batch.next_obs)
  ...
  def _train(self, replay: replay_lib.ReplayBuffer):
    ...
    new_state = self._model.update_normalizer(self._state.state,
                                              transitions.obs, transitions.act,
    ...
    best_val_score = self._evaluate(self._state.params, self._state.state,
                                    val_iterator)
    for epoch in range(self.num_epochs):
      ...
      eval_score = self._evaluate(self._state.params, self._state.state,
                                  val_iterator)
      val_scores.append(eval_score.mean().item())

      maybe_best_params = self._maybe_get_best_params(best_val_score,
                                                      eval_score)
