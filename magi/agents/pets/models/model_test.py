...
class ModelEnvTest(absltest.TestCase):
  ...
  def test_model_env(self):
    ...
    ensem_mean, ensem_std = ensemble.apply(ensem_params, key, observations,
                                           actions)
    ...
    self.assertEqual(unrolled_states.shape,
                     (num_timesteps, batch_size, obs_size))
