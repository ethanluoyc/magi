...
def ensemble_transform_with_state(fn, ensemble_size: int):
    ...
    def apply(params, state, rng, *args):
        ...
        return jax.vmap(fn_transformed.apply, in_axes=in_axes)(
            params, state, rngs, *args
        )
...
def _truncated_normal_w_init(shape, dtype):
    assert len(shape) == 2
    fan_in = shape[0]
    std = 1.0 / (2 * jnp.sqrt(fan_in))
    mean = jax.lax.convert_element_type(0.0, dtype)
    std = jax.lax.convert_element_type(std, dtype)
    unscaled = jax.random.truncated_normal(
        hk.next_rng_key(), shape=shape, lower=-2.0, upper=2.0
    )
