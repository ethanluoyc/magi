...
class SACV1Learner(core.Learner):
    ...
    def __init__(
        ...
        def alpha_loss_fn(log_alpha: jnp.ndarray, entropy: jnp.ndarray) -> jnp.ndarray:
        ...
        def critic_loss_fn(
            ...
            q1, q2 = critic_network.apply(critic_params, data.observation, data.action)
        ...
        def sgd_step(state: TrainingState, transitions: types.Transition):
            ...
            # Update critic
            (critic_loss, critic_metrics), critic_grads = critic_grad_fn(
                state.critic_params, state.target_value_params, transitions
            )
            ...
            target_value_params = polyak_update(value_params, state.target_value_params)
            ...
            if adaptive_entropy_coefficient:
                ...
                metrics.update(
                    {"alpha_loss": alpha_loss, "alpha": jnp.exp(alpha_params)}
                )
