...
from magi.agents.rhpo import builder as rhpo_builder
from magi.agents.rhpo import config as rhpo_config
from magi.agents.rhpo import networks as rhpo_networks


class RHPO(agent_lib.Agent):
  """RHPO agent."""

  builder: rhpo_builder.RHPOBuilder

  def __init__(
      self,
      environment_spec: specs.EnvironmentSpec,
      networks,
      config: rhpo_config.RHPOConfig,
      seed: int,
      num_components: int,
      num_tasks: int,
    ...
    self.builder = rhpo_builder.RHPOBuilder(
      config,
    ...
    actor = self.builder.make_actor(
        actor_key,
        rhpo_networks.apply_policy_and_sample(
