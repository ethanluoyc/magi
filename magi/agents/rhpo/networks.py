...
def make_networks(
  ...
  def _policy(obs, task_ids):
    ...
    # (T, B, L)
    mixture_logits = jnp.stack([head(embedding) for head in controller_heads])
    # (B, T)
    task_ids = jax.nn.one_hot(task_ids, num_classes=num_tasks)
    # (T, B, 1)
    task_ids = jnp.expand_dims(jnp.swapaxes(task_ids, 0, 1), axis=-1)
    # (B, L)
    task_mixture_logits = jnp.sum(task_ids * mixture_logits, axis=0)
  ...
  return {
      'policy':
          networks_lib.FeedForwardNetwork(
              init=lambda key: policy_network.init(key, dummy_obs, dummy_task_id
                                                  ),
              apply=policy_network.apply),
      'critic':
          networks_lib.FeedForwardNetwork(
              init=lambda key: critic_network.init(key, dummy_obs, dummy_act),
              apply=critic_network.apply),
  }
