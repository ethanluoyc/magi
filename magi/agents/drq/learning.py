...
def _critic_loss_fn(encoder_fn, policy_fn, critic_fn, gamma: float):
  ...
  def loss_fn(
    ...
    next_q1, next_q2 = critic(target_critic_params, data.next_observation,
                              next_actions)
...
def _alpha_loss_fn(log_alpha: jnp.ndarray, entropy: jnp.ndarray,
...
class DrQLearner(core.Learner):
  ...
  def __init__(
    ...
    actor_loss_fn = _actor_loss_fn(encoder_network.apply, policy_network.apply,
                                   critic_network.apply)
    alpha_loss_fn = functools.partial(
        _alpha_loss_fn, target_entropy=target_entropy)
    ...
    def sgd_step(
      ...
      transitions = transitions._replace(
          ...
          next_observation=augmentation(key_aug_next,
      ...
      new_params = optax.apply_updates(state.encoder_critic_params,
                                       critic_update)
    ...
    def make_initial_state(key):
      ...
      encoder_init_critic_opt_state = critic_optimizer.init({
          'encoder': encoder_init_params,
          'critic': critic_init_params
      })
