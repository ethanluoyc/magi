...
def alpha_loss_fn(log_alpha: jnp.ndarray, entropy: jnp.ndarray,
                  target_entropy: float) -> jnp.ndarray:
...
def critic_loss_fn(actor, critic, critic_params: hk.Params, key: jax.random.PRNGKey,
                   critic_target_params: hk.Params, actor_params: hk.Params,
                   log_alpha: jnp.ndarray, batch: reverb.ReplaySample, gamma: float):
  ...
  next_q1, next_q2 = critic.apply(critic_target_params, data.next_observation,
                                  next_actions)
