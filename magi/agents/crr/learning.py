...
class CRRLearner(acme.Learner):
  ...
  def __init__(
    ...
    def loss_fn(
      ...
      # ========================= Critic learning =============================
      q_tm1_logits, q_tm1_atoms = critic_network.apply(critic_params, o_tm1,
                                                       a_tm1)
      target_action_distribution = policy_network.apply(policy_target_params,
                                                        o_t)
      ...
      sampled_q_t_logits, sampled_q_t_atoms = jax.vmap(
          critic_network.apply, (None, None, 0))(critic_target_params, o_t,
                                                 sampled_actions_t)
      ...
      critic_loss_t = batch_td_learning(q_tm1_atoms, q_tm1_logits, r_t, d_t,
                                        q_t_atoms, q_t_logits)
      ...
      tiled_z_tm1_logits, tiled_z_tm1_values = jax.vmap(
          critic_network.apply, (None, None, 0))(critic_params, o_tm1,
                                                 action_tm1)
      tiled_v_tm1 = jax.vmap(critic_mean)(tiled_z_tm1_logits,
                                          tiled_z_tm1_values)
    ...
    def sgd_step(
      ...
      (policy_grads, critic_grads), metrics = jax.grad(
          loss_fn, has_aux=True)(online_params, target_params, batch, step_key)
      metrics.update({
      ...
      new_critic_params = optax.apply_updates(state.critic_params,
                                              critic_updates)
      new_policy_params = optax.apply_updates(state.policy_params,
                                              policy_updates)
